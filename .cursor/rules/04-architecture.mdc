...
## システム構成
Ruby製のCLIツールとして実装。
ルールは入力しやすいように、mdファイルとして作成し、コピーする際に拡張子をmdcに変更する。
mdファイル、mdcファイルは、 _は使えず、-のみ使用できる。
...
また、templatesフォルダ直下に新たなルールを作成する際の雛形となるrule-template.mdを配置する。
テストはできる限り細かく作成してください。
