...
def init_command(template_dir: Optional[str] = None, force: bool = False) -> bool:
    ...
    if template_dir:
        ...
        if not os.path.exists(template_path):
            utils.log_error(
                f"テンプレートディレクトリ '{template_dir}' が見つかりません"
            )
            return False
    else:
        ...
        for root, _, files in os.walk(rules_path):
            for file in files:
                if file.endswith(".md"):  # .mdファイルを処理
                    ...
                    # ファイル内容を検証
                    missing_fields = utils.validate_file_content(
                        source, ["description"]
                    )
                    if missing_fields:
                        utils.log_error(
                            f"必須フィールドがありません: {', '.join(missing_fields)} in {source}"
                        )
                        success = False
                        continue
        ...
        for root, _, files in os.walk(rules_path):
            for file in files:
                if file.endswith(".md"):  # .mdファイルを処理
                    ...
                    if utils.resolve_conflict(source, target, force):
                        utils.log_info(
                            f"編集済みルールを配置しました: {source} -> {target}"
                        )
                    else:
                        success = False
                    ...
                    utils.display_progress(
                        processed_files, total_files, "編集済みルールの配置"
                    )
...
def list_command(template_dir: Optional[str] = None, format: str = "text") -> bool:
    ...
    # 出力形式に応じて表示
    if format == "json":
        result = {"hierarchy": hierarchy, "rules": rules}
