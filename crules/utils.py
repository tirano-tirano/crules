...
def validate_file_content(
    file_path: Union[str, Path], required_fields: Optional[List[str]] = None
) -> List[str]:
    """
    ファイルの内容を検証します。
...
def validate_file_format(
    file_path: str, required_fields: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
...
def validate_file_size(file_path: str, max_size: int = 1024 * 1024) -> bool:
    ...
    if file_size > max_size:
        raise ValidationError(
            f"ファイルサイズが制限を超えています: {file_size} bytes " f"(最大: {max_size} bytes)"
        )
...
def validate_file_structure(
    file_path: str, required_sections: Optional[List[str]] = None
) -> bool:
    """
    ファイルの構造を検証します。
    ...
    if required_sections:
        ...
        if missing_sections:
            raise ValidationError(f"必須セクションが欠けています: {', '.join(missing_sections)}")
...
def get_directory_hierarchy_string(
    directory: Union[str, Path], prefix: str = "", is_last: bool = True
) -> str:
    """
    ...
    for i, item in enumerate(items):
        ...
        if item.is_dir():
            result.append(
                get_directory_hierarchy_string(item, next_prefix, i == len(items) - 1)
            )
        else:
...
def validate_yaml_front_matter(
    content: Union[str, Path], required_fields: Optional[List[str]] = None
) -> bool:
    """
